...
Lifted verbatim from schemablind's, which was lifted from moneytrail's. That is
deliberate -- these projects should tell one story about how a model gets
called, and every defensive line in here was paid for once already on a real
run. The only changes are the env prefix and the user agent.

This project calls it twice per turn, for the agent and for the simulated
customer, which makes the daily-quota handling matter more rather than less: a
run that dies halfway through trial 3 of 5 has no pass^5 to report at all.
...
PROVIDER_ENV = "RERUNS_PROVIDER"
MODEL_ENV = "RERUNS_MODEL"
USER_AGENT = "reruns/0.1"
